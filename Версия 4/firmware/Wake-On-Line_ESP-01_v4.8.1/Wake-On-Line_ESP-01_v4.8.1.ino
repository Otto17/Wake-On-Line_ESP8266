...
Удалённое отслеживание и управлением состояним ПК через WEB интерфейс или через приложение RemoteXY (Включается или Выключается из вкладки "a0_Settings" перед компиляцией).

  Возможности WEB:
    Страница "Управление":
      - Проверка состояния ПК (ON/OFF) [без обновления страницы];
      - Удалённое Вкл. и Выкл. ПК;
      - Удалённное жёсткое выключение ПК (в случае его зависания);
...
3) Посмотреть на роутере IP адрес, назначенный ESP (можно сразу же на роутере задать привязку статического IP через MAC).
4) Полдключаемся через WEB интерфейс (вводим Логин/Пароль для доступа нас страницу) и на вкладке "Настройки" жмём " Загрузить каталог" и загружаем: "gp_data" и "icons",
потом жмём " Загрузить файл" и в корень загружаем файл favicon. Жмём Ctrl+F5 (обновление страницы не обрращаясь в кеш).
5) Готово, пользуемся. Если Вкл. RemoteXY, то скачиваем его на смартфон и настраиваем.
...
Когда время работы портала закончится (время настраивается на вкладке "a0_Settings"), ESP перейдёт в режим STA (станция). Если не были вбиты настройки WiFI, то ESP будет пытыться подключиться к "пустой/несуществующей" сети,
...
При Вкл. форматировании FS для включения петребуется чуть больше времни (около 6 сек), следует ориентироваться на LED индикатор на модуле ESP.
...
Пароль: разрешён ввод от 4 до 20 символов (P.S. в поле пароль есть кнопка с глазом, что бы можно было посмотреть введённый пароль)
  ...
  1) Плата "Generic ESP8266 Module" (версия платы 3.1.2)
...
Изменения:
  - Начальный релиз (Версия: 4.8, Дата создания: 08.03.2023г.)
  - Обновил библиотеки и плату ESP8266 до версии 3.1.2 (Версия: 4.8.1, Дата создания: 10.06.2023г.)


  Дата создания: 10.06.2023г.
  Версия: 4.8.1
  Автор: Otto.
